...
namespace Project.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult LoginByVirtualStaff(string returnUrl)
        {
        ...
        //    List<AccessList> lstAccessList = db.AccessList.ToList();
        //    return View(lstAccessList);
        //}

        [HttpPost]
        [SessionTimeout]
        public ActionResult SetUserPermission(List<AccessList> model, int? UserRightID)
        {
            string info = "";
        ...
        [HttpGet]
        public ActionResult LoginByCompanyPerson(string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            return View();
        }
        ...
        private ActionResult RedirectToLocal(string returnUrl/*, int Type*/)
        {
            ...
            //    return this.RedirectToAction("CompanyDashboard", "Home");
            //}
            //else
            //{
        ...
        public ActionResult UserRightPermission(int? UID)//ActionNameAuthentication() {ActionNameID = Action.FirstOrDefault().ActionID
        {
            ...
            if (TempData["tmpListOfAccess"] != null)
            {
                ...
                ViewBag.EmployeeID = new SelectList(db.Employee.Select(s => new { EmployeeID = s.EmployeeID, Name = s.LoginName }).ToList(), "EmployeeID", "Name");

                lstAccessList = (List<AccessList>)TempData["tmpListOfAccess"];
            }
            else
            {
                ...
                ViewBag.EmployeeID = new SelectList(db.Employee.Select(s => new { EmployeeID = s.EmployeeID, Name = s.LoginName }).ToList(), "EmployeeID", "Name");
                var lstAccessLists = db.AccessList.Where(x => x.Status == 1).AsQueryable();
                int i = lstAccessList.Count();
                lstAccessList = lstAccessList.ToList();
                ...
                if (userRightPermission != null)
                {
                    ...
                    foreach (var item in lstAccessList)
                    {
        ...
        //    return View(model);
        //}
        //private List<ActionNameAuthentication> AddInList(IEnumerable<ActionList> action)
        //{
        ...
        public ActionResult AddUserRightName(string UserRightName)
        {
            ...
            try
            {
                ...
                userRightPermissionForInsert.CreateBy = AppUtils.GetLoginUserID();
        ...
        public ActionResult GetEmployeeStatusByEmployeeID(int EmployeeID)
        {
            ...
            if (employee != null)
            {
                return Json(new { Success = true, EmployeeID = EmployeeID, EmployeeStatusID = employee.Status }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult DeleteEmployeeByEmployeeID(int EmployeeID)
        {
            ...
            if (employee != null)
            {
                try
                {
                    employee.Status = AppUtils.TableStatusIsDelete;
                    db.Entry(employee).State = EntityState.Modified;
                    db.SaveChanges();
